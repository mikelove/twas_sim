...
def sim_gwas(L, ngwas, b_qtls, var_explained, eqtl_h2, n_snps):
    """
    ...
    :return: (pandas.DataFrame, float, numpy.ndarray) estimated GWAS beta and standard error, causal GE effect
    """
    Z_gwas = sim_geno(L, ngwas)

    # add horizontal pleiotropy via SNPs with trait-only effects
    b_qtls_gwas = b_qtls.copy()
    c_qtls = np.random.choice(len(b_qtls_gwas), 3) # pick 3 SNPs
    sign = np.array([-1,1])
    # give large effect size
    n_qtls = 0.01 * n_snps # 1pct model
    b_qtls_gwas[ c_qtls ] = 2 * np.random.choice(sign, 3) * np.sqrt(eqtl_h2 / n_qtls) 

    # var_explained should only reflect that due to genetics
    gwas_expr = np.dot(Z_gwas, b_qtls_gwas)
    if var_explained > 0:
...
def main(args):
    ...
    gwas, alpha, b_qtls_gwas = sim_gwas(L, args.ngwas, b_qtls, args.var_explained, args.eqtl_h2, p)
    ...
    output["gwas.true.hp"] = b_qtls_gwas * alpha
